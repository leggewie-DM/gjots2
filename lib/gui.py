...
class gjots_gui:
	...
	def _setup_gconf(self):
		...
		if not self.client.get_string(self.print_command_path):
			self.client.set_string(self.print_command_path, "if type gv; then GV='gv --orientation=seascape'; else if type ggv; then GV=ggv; else lpr $1; exit; fi; fi; T=/tmp/.tmp.$$;mpage -f -2 -I1 -P- $1 > $T;($GV $T; rm $T)&")
